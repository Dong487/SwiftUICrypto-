...
class DetailViewModel: ObservableObject{
    ...
    private func creatOverviewArray(coinModel: CoinModel) -> [StatisticModel]{
        ...
        let priceStat = StatisticModel(title: "當前價格", value: price, percentageChange: pricePercentChange)
        
        // marketCap 為可選的 如果沒有 則為空字串
        let marketCap = "$" + (coinModel.marketCap?.formattedWithAbbreviations() ?? "")
        let marketCapPercentChange = coinModel.marketCapChangePercentage24H
        let marketCapStat = StatisticModel(title: "總市值", value: marketCap, percentageChange: marketCapPercentChange)
    ...
    private func creatAdditionalArray(coinDetailModel: CoinDetailModel? ,coinModel: CoinModel) -> [StatisticModel]{
        ...
        let highStat = StatisticModel(title: "24 小時內 最高價", value: high)
        
        // 最低點
        let low = coinModel.low24H?.asCurrencyWith6Decimals() ?? "n/a"
        let lowStat = StatisticModel(title: "24 小時內 最低價", value: low)
        
        // 24H內變化量
        let priceChange = coinModel.priceChange24H?.asCurrencyWith6Decimals() ?? "n/a"
        let pricePercentChange = coinModel.priceChangePercentage24H
        let priceChangeStat = StatisticModel(title: "24 小時內 變化", value: priceChange, percentageChange: pricePercentChange)
        
        let marketCapChange = "$" + (coinModel.marketCapChange24H?.formattedWithAbbreviations() ?? "" )
        let marketCapPercentChange = coinModel.marketCapChangePercentage24H
        let marketChangeStat = StatisticModel(title: "24 小時內 Cap Change", value: marketCapChange, percentageChange: marketCapPercentChange)
